  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-[15px] font-semibold">Telegram bridge</h2>
        <span className="flex-1" />
        <button onClick={() => void load()} className="p-1.5 rounded text-white/40 hover:text-white hover:bg-white/[0.06] transition" title="Refresh"><RefreshCw className="w-3.5 h-3.5" /></button>
      </div>

      <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4 mb-4 text-[12.5px] text-white/70">
        Bot: <a href={`https://t.me/${info?.botUsername || 'your_bot'}`} target="_blank" rel="noopener" className="text-sky-300 hover:underline">@{info?.botUsername || 'your_bot'}</a>
        {info?.linked
          ? <span className="ml-2 text-emerald-300">● linked to the owner</span>
          : <span className="ml-2 text-amber-300">○ not linked — send the bot any message to lock it to yourself</span>}
        <div className="mt-2 text-white/45 text-[11.5px]">Message the bot a task — it runs on the VPS in the active thread. The conversation stays in sync with these sessions: continue any thread here by clicking "open".</div>
      </div>

      {err && <div className="text-red-300 text-[12px] mb-3">Error: {err}</div>}
        ...
        {(info?.threads || []).map((t) => (
          <div key={t.key} className={'rounded-lg border p-3 ' + (t.active ? 'border-sky-400/40 bg-sky-500/[0.06]' : 'border-white/[0.06] bg-white/[0.015]')}>
            <div className="flex items-center gap-2">
              ...
              {t.active && <span className="text-[10px] text-sky-300">active</span>}
              <span className="flex-1" />
            ...
            <div className="mt-2">
              {t.sessionId
                ? <button onClick={() => onOpen(t.sessionId!)} className="px-2.5 py-1 rounded text-[11px] bg-white/[0.05] hover:bg-sky-500/15 text-white/70 hover:text-sky-100 transition">↪ open conversation</button>
                : <span className="text-[11px] text-white/30">this thread has not been used yet</span>}
            </div>
          </div>
        ))}
      </div>

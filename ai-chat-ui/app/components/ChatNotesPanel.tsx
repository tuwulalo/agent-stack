...
export function ChatNotesPanel({ chatId, lastExchange, refreshKey, open, onOpenChange }: Props) {
  ...
  const doCreateNote = async () => {
    ...
    const raw = prompt('Имя файла (только латиница, цифры, _ -, без .md):', `note-${Date.now().toString(36)}`)
    if (!raw) return
    const name = raw.trim().replace(/\.md$/i, '') + '.md'
    if (!isValidNoteName(name)) {
      alert('Неподходящее имя. Разрешено: латиница, цифры, _, -, длина до 80, расширение .md')
      return
    }
            ...
            >
              <header className="flex items-center gap-2.5 px-4 py-3.5 border-b border-white/[0.06]">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[13px] font-medium text-white/90">Память чата</div>
              ...
              <div className="px-4 py-2.5 flex items-center gap-2 border-b border-white/[0.04]">
                <button
                  ...
                  title={lastExchange ? 'Попросить Kimi обновить facts/decisions/summary' : 'Нужен хотя бы один ответ от Kimi'}
                >
                  <RefreshCw className={'w-3 h-3 ' + (busy ? 'animate-spin' : '')} />
                  {busy ? 'Обновляю…' : 'Регенерить'}
                </button>
                <button
              ...
              <div className="flex-1 overflow-y-auto px-2 py-2">
                {loading ? (
                  <div className="px-2 py-6 text-center text-[12px] text-white/40">загрузка…</div>
                ) : notes.length === 0 ? (
                  <div className="px-3 py-8 text-center">
                    <Brain className="w-10 h-10 mx-auto text-white/15 mb-3" />
                    <div className="text-[12.5px] text-white/55 mb-1">Память пока пуста</div>
                    <div className="text-[10.5px] text-white/35 leading-relaxed">
                      После первого ответа Kimi автоматически сформирует facts.md / decisions.md / summary.md.
                    </div>
                  </div>
                ) : (
              ...
              <footer className="px-4 py-2.5 border-t border-white/[0.04] text-[10.5px] text-white/35 leading-snug">
                Эти MD автоматически инжектятся в system-prompt каждого ответа Kimi.
                Правь руками — будет учитываться сразу.
              </footer>
            </motion.aside>
          </>
        )}
